...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Parallel: run steps with multiprocessing",
    )
    parser.add_argument(
        "-v",
        "--verbose",
        action=argparse.BooleanOptionalAction,
        help="Verbose: print extra info about steps being run to terminal",
    )
    parser.add_argument(
        "-tid", "--target_id", type=str, default="L697741", help="Observation SAS id"
    )
    parser.add_argument(
        "-rid",
        "--reference_ids",
        nargs="+",
        type=str,
        default=["L697733", "L697735", "L697737", "L697739"],
        help="List of reference stations observations SAS ids",
    )
    parser.add_argument(
        "-i",
        "--input_dir",
        type=str,
        default="/data/vohl",
        help="Input data trunk directory",
    )
    parser.add_argument(
        "-o",
        "--output_dir",
        type=str,
        default="/data/vohl/output",
    ...
    parser.add_argument(
        ...
        "--to_disk",
        action=argparse.BooleanOptionalAction,
        help="Save to disk as HDF5",
    )
    parser.add_argument(
        "-gpu", "--use_gpu", action=argparse.BooleanOptionalAction, help="Use GPU(s)"
    )
    parser.add_argument(
        "-pyfftw",
        "--use_pyfftw",
        action=argparse.BooleanOptionalAction,
        help="Use pyfftw; if False, uses numpy",
    )
    parser.add_argument(
        "-numba",
        "--use_numba",
        action=argparse.BooleanOptionalAction,
        help="Use numba: if False, runs default single-threaded python",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Number of CPU cores to use when --parallel is used. (default: 2)",
    )
    ...
    if verbose:
        print(
            f"parallel:{parallel}, verbose:{verbose}, to_disk:{to_disk}, use_gpu:{use_gpu}, use_pyfftw:{use_pyfftw}"
        )
    ...
    f = h5py.File(
        f"{trunk_dir}{reference_ids[0]}/{cs_str}/{reference_ids[0]}_SAP000_B000_S0_P000_bf.h5",
        "r",
    )
    ...
    xcorr_threshold_shrink_power = (
        0.05  # How big are the regions where the local rms is computed.
    )
    xcorr_cpus = ncpus
    ...
    plot_beam_beam = 0
    plot_beam_ffun = (
        lambda ref, refbm, spw: f"{target_id}_xcorr/{ref}/{refbm}/SAP000_B{plot_beam_beam:03d}_P000_spw{spw}_avg{xcorr_dt}.h5"
    )
    ...
    average_t_dt = 60
    average_t_weighted = (
        True  # Use the weights when averaging the data in time. Set to True.
    )
    average_t_cpus = 20
    ...
    solve_uvhol_pols = [
        "XX",
        "YY",
    ]  # Polarizations for which we want phase and amplitude values. Do 'XX' and 'YY' only, as the cross terms should contain noise.
    solve_uvhol_spws = range(0, spws)
    solve_weighted = (
        True  # Use wights when solving for the amplitudes and phases of the stations?
    )

    # Order_sols step options.
    # Puts all the solutions for one polarization in a single file.
    order_sols_phase_reference_station = (
        "CS026HBA1"  # Station used to reference the phases.
    )
    ...
    logging.basicConfig(
        filename=logfile, filemode="a", level=logging.DEBUG, format=formatter
    )
