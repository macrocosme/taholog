...
from taholog.testing.checks import (
    check_channelized_file_count,
    check_correlated_file_count,
)
...
def run_pipeline(params, verbose=False):
    ...
    check_channelized_file_count(
        logger, output_dir, target_id, reference_ids, params, verbose
    )
    ...
    if "xcorr" in steps:
        procs._xcorr(output_dir, reference_ids, target_id, xcorr_dt, params, verbose)

    _continue, missing = check_correlated_file_count(
        logger,
        output_dir,
        target_id,
        reference_ids,
    ...
    if "gencal" in steps:
        procs._gencal(
            output_dir, target_id, xcorr_dt, reference_ids, params, parallel, verbose
        )
        procs._gencal(
            output_dir, target_id, xcorr_dt, reference_ids, params, parallel, verbose
        )
    ...
    if "applycal" in steps:
        procs._applycal(
            output_dir, target_id, xcorr_dt, params, reference_ids, parallel, verbose
        )
        procs._applycal(
            output_dir, target_id, xcorr_dt, params, reference_ids, parallel, verbose
        )
