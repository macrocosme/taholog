...
def _to_freq(
    ...
    for ref in reference_ids:
        ...
        _outdir = check_folder_exists_or_create(
            f"{output_dir}{ref}", return_folder=True
        )
        ...
        for beam in params["ref_beams"]:
            ...
            outdir = check_folder_exists_or_create(
                f"{_outdir}{beam}", return_folder=True
            )
    ...
    outdir = check_folder_exists_or_create(
        f"{output_dir}{target_id}", return_folder=True
    )
    ...
    if not parallel:
        for current_dir, input_file, output_base in zip(
            current_directories, input_files, output_bases
        ):
            to_freq.main(
                input_file,
                output_base,
...
def _xcorr(output_dir, reference_ids, target_id, xcorr_dt, params, verbose=False):
    ...
    if not os.path.isdir(xcorr_output_dir):
        ...
        for ref in reference_ids:
            check_folder_exists_or_create(
                f"{xcorr_output_dir}/{ref}", return_folder=False
            )
            for ref_beam in params["ref_beams"]:
                check_folder_exists_or_create(
                    f"{xcorr_output_dir}/{ref}/{ref_beam}", return_folder=False
                )
    ...
    rfi_kwargs = {
        "flagging_threshold": params["xcorr_flagging_threshold"],
...
def _redo_missing_xcorr(
    output_dir, xcorr_output_dir, target_id, params, xcorr_dt, refid, ref_beam, spw, ibm
):
    ...
    """
    target = (
        lambda _ibm, _spw: f"{output_dir}{target_id}/{target_id}_SAP000_B{_ibm:03d}_P000_bf_spw{_spw}.h5"
    )
    refers = (
        lambda _ref, _refbm, _spw: f"{output_dir}{_ref}/{_refbm}/{_ref}_SAP000_B000_P000_bf_spw{_spw}.h5"
    )
    output = (
        lambda _ref, _refbm, _ibm, _spw: f"{xcorr_output_dir}/{_ref}/{_refbm}/SAP000_B{_ibm:03d}_P000_spw{_spw}_avg{xcorr_dt}.h5"
    )
    rfi_output = (
        lambda _ref, _refbm, _ibm, _spw: f"{xcorr_output_dir}/{_ref}/{_refbm}/SAP000_B{_ibm:03d}_P000_spw{_spw}_rfiflags.h5"
    )
...
def _gencal(
    output_dir, target_id, xcorr_dt, reference_ids, params, parallel, verbose=False
):
    ...
    if verbose:
        print("gencal")
    target = (
        lambda ref, refbm, spw: f"{output_dir}{target_id}_xcorr/{ref}/{refbm}/SAP000_B000_P000_spw{spw}_avg{xcorr_dt}.h5"
    )
    output = (
        lambda ref, refbm, spw: f"{output_dir}{target_id}_xcorr/{ref}/{refbm}/SAP000_B000_P000_spw{spw}_avg{xcorr_dt}_sol.h5"
    )
...
def _applycal(
    output_dir, target_id, xcorr_dt, params, reference_ids, parallel, verbose=False
):
    """Apply calibration
    ...
    target = (
        lambda ref, refbm, beam, spw: f"{output_dir}{target_id}_xcorr/{ref}/{refbm}/SAP000_B{beam:03d}_P000_spw{spw}_avg{xcorr_dt}.h5"
    )
    output = (
        lambda ref, refbm, beam, spw: f"{output_dir}{target_id}_xcorr/{ref}/{refbm}/SAP000_B{beam:03d}_P000_spw{spw}_avg{xcorr_dt}_cal.h5"
    )
            ...
            for ref_beam in params["ref_beams"]:
                for spw in params["applycal_spws"]:
                    ...
                    for beam in params["applycal_beams"]:
                        ...
                        pool.apply_async(applycal.main, args=(solutions_file, tgt, out))
        pool.close()
        pool.join()
...
def _clip(output_dir, target_id, reference_ids, xcorr_dt, params, parallel):
    ...
    output = (
        lambda ref, refbm, beam, spw: f"{output_dir}{target_id}_xcorr/{ref}/{refbm}/SAP000_B{beam:03d}_P000_spw{spw}_avg{xcorr_dt}_cal_clip.h5"
    )
            ...
            for ref_beam in params["ref_beams"]:
                for spw in params["clip_spws"]:
                    for beam in params["clip_beams"]:
                        ...
                        pool.apply_async(
                            clip.abs_clip_vis, args=(tgt, out), kwds=kwargs
                        )
        pool.close()
        pool.join()
...
def _average_t(
    output_dir, target_id, average_t_dt, reference_ids, xcorr_dt, params, parallel
):
    """Average in time
    ...
    output = (
        lambda ref, refbm, beam, spw: f"{output_dir}{target_id}_xcorr/{ref}/{refbm}/SAP000_B{beam:03d}_P000_spw{spw}_avg{xcorr_dt}_cal_clip_avg{average_t_dt}.h5"
    )

    kwargs = {"target_time_res": average_t_dt, "weighted": params["average_t_weighted"]}
            ...
            for ref_beam in params["ref_beams"]:
                for spw in params["average_t_spws"]:
                    for beam in params["average_t_beams"]:
                        ...
                        pool.apply_async(
                            average.time_average_vis, args=(tgt, out), kwds=kwargs
                        )
        pool.close()
        pool.join()
...
def _to_uvhol(
    output_dir, target_id, xcorr_dt, average_t_dt, reference_ids, params, parallel
):
    ...
    """
    target = (
        lambda ref, refbm, spw: f"{output_dir}{target_id}_xcorr/{ref}/{refbm}/SAP000_B*_P000_spw{spw}_avg{xcorr_dt}_cal_clip_avg{average_t_dt}.h5"
    )
    output = (
        lambda ref, refbm, spw: f"{output_dir}{target_id}_xcorr/{ref}/{refbm}/spw{spw}_avg{xcorr_dt}_cal_clip_avg{average_t_dt}"
    )
...
def _average_uvhol(
    output_dir, target_id, xcorr_dt, average_t_dt, params, reference_ids
):
    """Average uvhol
        ...
        for spw in params["average_uvhol_spws"]:
            ...
            for refid in reference_ids:
                for ref_beam in params["ref_beams"]:
                    ...
                    file_list.append(
                        glob.glob(
                            f"{output_dir}{target_id}_xcorr/{refid}/{ref_beam}/*spw{spw}_avg{xcorr_dt}_cal_clip_avg{average_t_dt}_{pol}_t*.uvhol"
                        )
                    )
            ...
            output = (
                lambda t: f"{output_dir}{target_id}_xcorr/spw{spw}_avg{xcorr_dt}_cal_clip_avg{average_t_dt}_{pol}_t{t}.uvhol"
            )
...
def _solve_uvhol(output_dir, target_id, xcorr_dt, average_t_dt, params, logger):
    ...
    for pol in params["solve_uvhol_pols"]:
        for spw in params["solve_uvhol_spws"]:
            ...
            files = glob.glob(
                f"{output_dir}{target_id}_xcorr/spw{spw}_avg{xcorr_dt}_cal_clip_avg{average_t_dt}_{pol}_t*.uvhol"
            )
...
def _order_sols(output_dir, target_id, xcorr_dt, average_t_dt, params):
    ...
    for pol in params["solve_uvhol_pols"]:
        sol_files = glob.glob(
            f"{output_dir}{target_id}_xcorr/spw*_avg{xcorr_dt}_cal_clip_avg{average_t_dt}_{pol}_t*.pickle"
        )
        output = f"{output_dir}{target_id}_xcorr/avg{xcorr_dt}_cal_clip_avg{average_t_dt}_{pol}_sols.pickle"
        order.main(
            sol_files,
            output,
            params["order_sols_phase_reference_station"],
            params["order_sols_degree"],
        )
...
def _plot_report(output_dir, target_id, xcorr_dt, average_t_dt, params):
    ...
    for pol in params["solve_uvhol_pols"]:
        ...
        uvhol_files_func = (
            lambda spw: f"{output_dir}{target_id}_xcorr/spw{spw}_avg{xcorr_dt}_cal_clip_avg{average_t_dt}_{pol}_t0.uvhol"
        )
        plot.plot_report(
            params["plot_report_output"](pol),
            solutions_file,
            uvhol_files_func,
            params["order_sols_phase_reference_station"],
        )
