...
@njit(
    complex64[:, :](
        complex64[:, :, :], complex64[:, :, :, :], int64, int64, int64, int64, int64
    ),
    parallel=True,
)
def cross_correlate_jit(dt, dr, dt_j: int, dr_i: int, dr_k: int, ch0: int, chf: int):
...
def estimate_error(xcorr, xcorr_avgt0_shape, averaging_factor):
    ...
    for t in prange(xcorr_avgt0_shape[0]):
        ...
        drz[t] = np.std(xcorr[t0_:tf_, :, :, :].real, axis=0) / np.sqrt(
            xcorr[t0_:tf_, :, :, :].shape[0] - 1.0, dtype=np.float64
        )
        diz[t] = np.std(xcorr[t0_:tf_, :, :, :].imag, axis=0) / np.sqrt(
            xcorr[t0_:tf_, :, :, :].shape[0] - 1.0, dtype=np.float64
        )
...
def determine_averaging_factor(ntimes, target_time_res, integration_s):
    ...
    averaging_factor = averaging_values[
        np.argmin(abs(int(round(target_time_res / integration_s)) - averaging_values))
    ]
    return averaging_factor


def save_flags_hdf5(
    output, data, freq, target_head, reference_antenna, time_resolution
):
    """
...
def initialize(ibm, spw, refid, ref_beam):
    ...
    # Load reference data.
    dr = np.array(
        [fr["/{0}/DATA".format(b)].get("data") for b in fr.keys()], dtype=np.complex64
    )
...
def main(
    ...
    Args:
        target (str): Path to the target data file.
        reference (str): Path to the reference data file.
        output (str): Path to save the output data file.
        target_time_res (float, optional): Desired time resolution for averaging the data. Defaults to 0.
        rfiflag (bool, optional): Flag to enable RFI flagging. Defaults to False.
        edges (float, optional): Fraction of edge channels to remove. Defaults to 0.25.
        rfi_output (str, optional): Path to save the RFI flag masks. Defaults to "".
        rfi_kwargs (dict, optional): Additional keyword arguments for RFI flagging. Defaults to {}.
        parallel (bool, optional): Flag to enable parallel processing. Defaults to True.
        use_numba (bool, optional): Flag to enable Numba acceleration. Defaults to False.
    ...
    xcorr[:, :, 0, 0] = cross_correlate(
        dt[:], dr, 0, 0, 0, ch0, chf, parallel, use_numba
    )  # XX
    xcorr[:, :, 0, 1] = cross_correlate(
        dt[:], dr, 0, 0, 1, ch0, chf, parallel, use_numba
    )  # XY
    xcorr[:, :, 1, 0] = cross_correlate(
        dt[:], dr, 1, 0, 0, ch0, chf, parallel, use_numba
    )  # YX
    xcorr[:, :, 1, 1] = cross_correlate(
        dt[:], dr, 1, 0, 1, ch0, chf, parallel, use_numba
    )  # YY
    ...
    if target_time_res > 0:
        ...
        try:
            ...
            logger.warning(
                "Integration time not present in header. Will determine from observing time and number of time slots."
            )
            t0 = ht["mjd_start"]
            tf = ht["mjd_end"]
            obs_time = (tf - t0) * 24 * 3600.0
            integration_s = obs_time / ntimes

        logger.info(
            "Input data has an integration time of: {0} s".format(integration_s)
        )

        # Determine the averaging factor to reach the desired time resolution.
        averaging_factor = determine_averaging_factor(
            ntimes, target_time_res, integration_s
        )
        time_res = integration_s * averaging_factor

        logger.info(
            "Averaging data in time by a factor of: {0}".format(averaging_factor)
        )
        ...
        if rfiflag:
            ...
            flag_window_lengths = 2 ** np.arange(
                int(np.ceil(np.log(length_of_max_dimension) / np.log(2.0)))
            )

            # Flag each polarization product independently.
            logger.info("flagging each polarization.")
            flags_xx = flag.rfi_flag(
                abs(xcorr_avgt0[:, :, 0, 0]),
                flagging_threshold,
                flag_window_lengths,
                threshold_shrink_power,
            )
            flags_xy = flag.rfi_flag(
                abs(xcorr_avgt0[:, :, 0, 1]),
                flagging_threshold,
                flag_window_lengths,
                threshold_shrink_power,
            )
            flags_yx = flag.rfi_flag(
                abs(xcorr_avgt0[:, :, 1, 0]),
                flagging_threshold,
                flag_window_lengths,
                threshold_shrink_power,
            )
            flags_yy = flag.rfi_flag(
                abs(xcorr_avgt0[:, :, 1, 1]),
                flagging_threshold,
                flag_window_lengths,
                threshold_shrink_power,
            ...
            flags_xx = flag.extend_flags(
                flags_xx, time_percent=time_percent, freq_percent=freq_percent
            )
            flags_xy = flag.extend_flags(
                flags_xy, time_percent=time_percent, freq_percent=freq_percent
            )
            flags_yx = flag.extend_flags(
                flags_yx, time_percent=time_percent, freq_percent=freq_percent
            )
            flags_yy = flag.extend_flags(
                flags_yy, time_percent=time_percent, freq_percent=freq_percent
            )
            ...
            for i in range(rfi_kwargs["n_rfi_max"]):
                xcorr_avgt0[:, :, 0, 0].mask[
                    np.unravel_index(
                        np.ma.argmax(abs(xcorr_avgt0[:, :, 0, 0])),
                        xcorr_avgt0[:, :, 0, 0].shape,
                    )
                ] = True
                xcorr_avgt0[:, :, 1, 1].mask[
                    np.unravel_index(
                        np.ma.argmax(abs(xcorr_avgt0[:, :, 1, 1])),
                        xcorr_avgt0[:, :, 1, 1].shape,
                    )
                ] = True
            ...
            if rfi_output != "":
                ...
                save_flags_hdf5(
                    ...
                    tgt_freq,
                    ht,
                    hr["reference_antennas"],
                    time_res,
                )

        # Compute weights based on the error.
        wrz = np.ma.power(drz, -2.0)
        wiz = np.ma.power(diz, -2.0)
        ...
        logger.info(
            "Frequency averaged data has a shape of: {0}".format(xcorr_avg.shape)
        )
        ...
        weighted_sigma_re = csqrt(
            n_points
            / (n_points - 1.0)
            * np.ma.sum(
                wrz * np.power(xcorr_avgt0.real - xcorr_avg[:, np.newaxis], 2.0), axis=1
            )
            / np.ma.sum(wrz, axis=1)
        )
        weighted_sigma_im = csqrt(
            n_points
            / (n_points - 1.0)
            * np.ma.sum(
                wiz * np.power(xcorr_avgt0.imag - xcori_avg[:, np.newaxis], 2.0), axis=1
            )
    ...
    xcorr = xcorr_avg + 1.0j * xcori_avg
    xcorr = np.ma.masked_invalid(xcorr)
    sigma = weighted_sigma_re + 1.0j * weighted_sigma_im
