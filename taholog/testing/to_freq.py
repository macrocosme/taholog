...
to_freq_num_pol = (
    2  # Number of polarizations recorded by the backend. (Complex: real-imaginary)
)
            ...
            if use_gpu:
                ...
                for use_pyfftw in (False, True):
                    for to_disk in (True, False):
                        idx = (a, use_gpu, use_pyfftw, to_disk)
                        print(
                            f"method:{a}, {to_numpy_pyfftw(use_pyfftw)}, to_disk:{to_disk}"
                        )
                        timing[idx] = []
                        t = time_func(
                            approaches[a],
...
def time_approaches_multiple_files(
    ncpus,
    n_gpu_devices,
    input_file,
    output_base,
