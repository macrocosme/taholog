...
if continuing:
    ...
    rfi_kwargs = {'flagging_threshold': xcorr_flagging_threshold,
    ...
    def initialize(ibm, spw, refid, ref_beam):
        ...
        # Load reference data.
        dr = np.array([fr[f'/{b}/DATA'].get('data') for b in fr.keys()], dtype=np.complex64)
        ...
        return fr, ft, xcorr, radec, tgt_freq, ref_freq, dr, dt, ntimes, nchans, ch0, chf, beam_info
    ...
    fr, ft, xcorr, radec, tgt_freq, ref_freq, dr, dt, ntimes, nchans, ch0, chf, beam_info = initialize(ibm, spw, refid, ref_beam)
